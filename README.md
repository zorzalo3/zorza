...
## Setup
Install Python 3.8 or later and set up a Python virtual environment, then
install Django 4.2.28 or later (you can use `pip install -r requirements.txt`).
...
The root (`/`) url is an alias for `/pages/home/`, which is a [Django flatpage](https://docs.djangoproject.com/en/4.2/ref/contrib/flatpages/). Create a flatpage with the URL `/home/` in Django administration (`/admin/`), otherwise you will get a 404. You have to create an account first with `./manage.py createsuperuser`.
